...
function pedirIngrediente({ onOk }) {
    ...
    _abrirMassaModal(modal, () => {
        ...
        const valor = parseFloat(document.getElementById('mi-valor')?.value.replace(',', '.') || '0');
...
function _abrirMassaModal(modal, onConfirm) {
    ...
    const overlayHandler = e => { if (e.target === modal) { fechar(); modal.removeEventListener('click', overlayHandler); } };
    ...
    function bindEvents() {
        ...
        document.querySelectorAll('.massa-ingr-input').forEach(input => {
            input.addEventListener('change', () => {
                const id  = parseInt(input.dataset.mid);
        ...
        document.getElementById('massa-btnCopy')?.addEventListener('click', () => {
            ...
            copiarParaClipboard([`🍕 ${r.nome.toUpperCase()} — ${k} kg de trigo`, '', ...linhas].join('\n'));
        });
    }
