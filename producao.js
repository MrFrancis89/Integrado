...
export function renderProducao() {
    ...
    if (receitas.length === 0) {
        ...
        aviso.className = 'prod-vazio';
        aviso.textContent = 'Nenhuma receita encontrada. Crie receitas na aba Massa primeiro.';
    ...
    const configCard = _criarCard('⚙️ Configurar Produção');
    ...
    btnCalc.textContent = '📊 Calcular Produção';
    secao.appendChild(btnCalc);

    // ── Seção: Tabela resumo (oculta inicialmente) ────────────────
    const resumoCard = _criarCard('📊 Resumo da Produção');
    ...
    const sepCard = _criarCard('📦 Lista de Separação');
    ...
    btnCopy.textContent      = '📋 Copiar Lista de Separação';
